#include <h2opus/core/horthog.h>
#include <h2opus/marshal/horthog_marshal.h>

#include <h2opus/distributed/comm_wrapper.h>
#include <h2opus/distributed/distributed_horthog.h>
#include <h2opus/distributed/distributed_horthog_communication.h>
#include <h2opus/distributed/distributed_horthog_marshal.cuh>

#include <h2opus/util/batch_wrappers.h>
#include <h2opus/util/blas_wrappers.h>
#include <h2opus/util/debug_routines.h>
#include <h2opus/util/gpu_err_check.h>
#include <h2opus/util/thrust_wrappers.h>
#include <h2opus/util/timer.h>

#include <thread>
#include <mutex>
#include <condition_variable>

/////////////////////////////////////////////////////////////////////////////////////////////////////////
// Utility routines
/////////////////////////////////////////////////////////////////////////////////////////////////////////
void distributed_horthog_get_level_projection_dims(BasisTreeLevelData &level_data, HorthogWorkspace &branch_workspace,
                                                   int level, int &rows, int &cols)
{
    int level_te_rows;
    if (level < level_data.depth - 1)
    {
        int max_children = level_data.max_children;
        level_te_rows = max_children * branch_workspace.u_new_ranks[level + 1];
    }
    else
        level_te_rows = level_data.leaf_size;

    cols = level_data.getLevelRank(level);
    rows = std::min(level_te_rows, cols);
}

/////////////////////////////////////////////////////////////////////////////////////////////////////////
// Template routines
/////////////////////////////////////////////////////////////////////////////////////////////////////////
template <int hw>
void distributed_horthog_project_offdiagonal_template(
    TDistributedHNodeTree<hw> &dist_hnodes, TDistributedBasisTree<hw> &u_basis_tree,
    std::vector<HorthogLowRankExchangeHelper<hw>> &low_rank_exchange_helpers,
    DistributedHorthogWorkspace &dist_workspace, H2OpusEvents &events, h2opusComputeStream_t stream)
{
    THNodeTree<hw> &hnodes = dist_hnodes.off_diagonal_blocks;
    TBasisTree<hw> &basis_tree = u_basis_tree.basis_branch;
    HNodeTreeLevelData &hnode_level_data = hnodes.level_data;
    HorthogWorkspace &workspace = dist_workspace.branch_workspace;

    int num_levels = hnodes.depth;
    int top_level = 1; // basis_tree.level_data.nested_root_level;
    size_t max_nodes = hnode_level_data.getMaxLevelCouplingNodes();
    size_t increment = std::min((size_t)PROJECTION_MAX_NODES, max_nodes);

#ifdef H2OPUS_PROFILING_ENABLED
    Timer<hw> timer;
    timer.init();
    PerformanceCounter::clearCounters();

    timer.start();
#endif

    // Now go through the levels of the tree and compute the projection
    // of the coupling matrices into the new basis
    for (int level = num_levels - 1; level >= top_level; level--)
    {
        size_t u_level_start = basis_tree.level_data.getLevelStart(level);
        size_t v_level_start = 0;
        H2Opus_Real *Tu_level = workspace.Tu_hat[level];
        H2Opus_Real *Tv_level =
            (H2Opus_Real *)low_rank_exchange_helpers[level].comm_buffer->getRecvAccumulationBuffer();

        events.streamWaitEvent<hw>(H2OpusBufferUpEvent, stream, level);

        horthog_project_level(hnodes, level, u_level_start, v_level_start, Tu_level, Tv_level,
                              vec_ptr(hnodes.node_u_index), vec_ptr(dist_hnodes.compressed_v_index), increment,
                              workspace, stream);
    }

#ifdef H2OPUS_PROFILING_ENABLED
    HLibProfile::cumRun(HLibProfile::HORTHOG_PROJECTION, PerformanceCounter::getOpCount(), timer.stop());
    PerformanceCounter::clearCounters();
#endif
}

template <int hw>
void distributed_horthog_project_diagonal_template(TDistributedHNodeTree<hw> &dist_hnodes,
                                                   TDistributedBasisTree<hw> &basis_tree,
                                                   DistributedHorthogWorkspace &dist_workspace,
                                                   h2opusComputeStream_t stream)
{
    // Project the diagonal block
    BasisTreeLevelData &basis_level_data = basis_tree.basis_branch.level_data;

#ifdef H2OPUS_PROFILING_ENABLED
    Timer<hw> timer;
    timer.init();
    PerformanceCounter::clearCounters();

    timer.start();
#endif

    horthog_project(dist_hnodes.diagonal_block, 1, basis_level_data.depth - 1, basis_level_data, basis_level_data,
                    dist_workspace.branch_workspace, stream);

#ifdef H2OPUS_PROFILING_ENABLED
    HLibProfile::cumRun(HLibProfile::HORTHOG_PROJECTION, PerformanceCounter::getOpCount(), timer.stop());
    PerformanceCounter::clearCounters();
#endif
}

template <int hw>
void distributed_horthog_process_toplevel_template(TDistributedHMatrix<hw> &dist_hmatrix,
                                                   DistributedHorthogWorkspace &dist_workspace, H2OpusEvents &events,
                                                   TDistributedTransferBuffer<hw> &gather_buffer,
                                                   h2opusComputeStream_t stream, int rank, int local_rank)
{
    h2opusSetDevice<hw>(local_rank);

    TBasisTree<hw> &basis_branch = dist_hmatrix.basis_tree.basis_branch;
    HorthogWorkspace &branch_workspace = dist_workspace.branch_workspace;

    //////////////////////////////////////////////////////////////////////////////////////////
    // Gather the root nodes of the all branches into the bottom level of the root branch
    //////////////////////////////////////////////////////////////////////////////////////////
    events.streamWaitEvent<hw>(H2OpusUpsweepEvent, stream, 0);

    H2Opus_Real *Tu_hat_branch_root = branch_workspace.Tu_hat[0];
    int top_level_proj_rows, top_level_proj_cols;
    distributed_horthog_get_level_projection_dims(basis_branch.level_data, branch_workspace, 0, top_level_proj_rows,
                                                  top_level_proj_cols);
    size_t top_elements = top_level_proj_rows * top_level_proj_cols;
    H2Opus_Real *Tu_hat_leaf_level = NULL;

    if (rank == 0)
        Tu_hat_leaf_level = dist_workspace.top_level_workspace.Tu_hat[dist_hmatrix.basis_tree.top_level.depth - 1];

    gather_buffer.prepareDownBuffer(Tu_hat_branch_root, stream);
    gather_buffer.setTransferBuffers(Tu_hat_branch_root, Tu_hat_leaf_level);
    gather_buffer.setTransferElements(top_elements, H2OPUS_MPI_REAL);

    events.recordEvent<hw>(H2OpusCommunicationEvent, H2OPUS_DIST_HORTHOG_GATHER_EVENT, stream);
    events.synchEvent<hw>(H2OpusCommunicationEvent, H2OPUS_DIST_HORTHOG_GATHER_EVENT);

    mpiErrchk(MPI_Gather(gather_buffer.getDownBuffer(), gather_buffer.getElements(), gather_buffer.getType(),
                         gather_buffer.getUpBuffer(), gather_buffer.getElements(), gather_buffer.getType(), 0,
                         gather_buffer.getComm()));

    if (rank == 0)
    {
        //////////////////////////////////////////////////////////////////////////////////////////
        // Upsweep
        //////////////////////////////////////////////////////////////////////////////////////////
        // Prepare the upsweep buffer
        BasisTreeLevelData &level_data = dist_hmatrix.basis_tree.top_level.level_data;
        HorthogWorkspace &top_workspace = dist_workspace.top_level_workspace;
        H2Opus_Real *Tu_hat_leaf_level = top_workspace.Tu_hat[level_data.depth - 1];
        gather_buffer.prepareUpBuffer(Tu_hat_leaf_level, stream);

#ifdef H2OPUS_PROFILING_ENABLED
        Timer<hw> timer;
        timer.init();
        PerformanceCounter::clearCounters();

        timer.start();
#endif

        // Now we can finish sweeping up the tree
        horthog_upsweep(dist_hmatrix.basis_tree.top_level, top_workspace, top_workspace.Tu_hat,
                        top_workspace.u_new_ranks, stream);

#ifdef H2OPUS_PROFILING_ENABLED
        HLibProfile::cumRun(HLibProfile::HORTHOG_UPSWEEP, PerformanceCounter::getOpCount(), timer.stop());
        PerformanceCounter::clearCounters();
        timer.start();
#endif

// TODO WAJIH: stitch
#if 0
        ////////////////////////////////////////////////////
        // Project the top level transfer nodes if necessary
        ////////////////////////////////////////////////////
        horthog_stitch(dist_hmatrix.basis_tree.top_level, top_workspace, top_workspace.Tu_hat,
                        top_workspace.u_new_ranks, stream);

#ifdef H2OPUS_PROFILING_ENABLED
        HLibProfile::cumRun(HLibProfile::HORTHOG_STITCH, PerformanceCounter::getOpCount(), timer.stop());
        PerformanceCounter::clearCounters();
        timer.start();
#endif
#endif
        //////////////////////////////////////////////////////////////////////////////////////////
        // Projection
        //////////////////////////////////////////////////////////////////////////////////////////
        horthog_project(dist_hmatrix.hnodes.top_level, 0, level_data.depth - 1, level_data, level_data, top_workspace,
                        stream);

#ifdef H2OPUS_PROFILING_ENABLED
        HLibProfile::cumRun(HLibProfile::HORTHOG_PROJECTION, PerformanceCounter::getOpCount(), timer.stop());
        PerformanceCounter::clearCounters();
#endif
    }
}

template <int hw>
void distributed_horthog_prepare_offDiag_template(TBasisTree<hw> &basis_branch,
                                                  TDistributedCompresseBasisTree<hw> &v_basis_tree,
                                                  TDistributedSendRecvBuffer<hw> *comm_buffer,
                                                  DistributedHorthogWorkspace &dist_workspace, int level,
                                                  h2opusComputeStream_t stream)
{
    if (level == 0)
        return;

    TDistributedCompressedBSNData<hw> &bsn_data = v_basis_tree.coupling_compressed_bsn_data[level];
    size_t num_nodes = bsn_data.send_process_nodes.size();

    H2Opus_Real **Tu_hat_ptrs = dist_workspace.ptr_A, **buffer_ptrs = dist_workspace.ptr_B;

    int proj_rows, proj_cols;
    distributed_horthog_get_level_projection_dims(basis_branch.level_data, dist_workspace.branch_workspace, level,
                                                  proj_rows, proj_cols);

    // Marshall all the block data
    H2Opus_Real *send_buffer = (H2Opus_Real *)comm_buffer->getSendAccumulationBuffer();
    H2Opus_Real *Tu_hat_level = dist_workspace.branch_workspace.Tu_hat[level];

    distributed_horthog_offdiag_projection_buffer_marshal<H2Opus_Real, hw>(
        Tu_hat_level, send_buffer, proj_rows, proj_cols, vec_ptr(bsn_data.send_process_nodes), Tu_hat_ptrs, buffer_ptrs,
        num_nodes, stream);

    // Copy the nodes into the buffer using the marshalled data
    check_kblas_error((H2OpusBatched<H2Opus_Real, hw>::copyBlock)(stream, proj_rows, proj_cols, buffer_ptrs, 0, 0,
                                                                  proj_rows, Tu_hat_ptrs, 0, 0, proj_rows, num_nodes));

    comm_buffer->prepareSendBuffer(stream);
}

template <int hw>
void distributed_horthog_low_rank_exchange(TBasisTree<hw> &basis_branch,
                                           TDistributedCompresseBasisTree<hw> &v_basis_tree,
                                           DistributedHorthogWorkspace &dist_workspace, H2OpusEvents &events,
                                           std::vector<HorthogLowRankExchangeHelper<hw>> &low_rank_exchange_helpers,
                                           std::mutex &upsweep_mutex, std::condition_variable &upsweep_cv,
                                           std::vector<int> &level_processed, int local_rank,
                                           h2opusComputeStream_t stream, int phase)
{
    // This may be run inside a thread, so make sure the device is set
    h2opusSetDevice<hw>(local_rank);
    int num_levels = basis_branch.depth;

    if (phase <= 0)
    { /* issue nonblocking calls */
        for (int level = num_levels - 1; level >= 1; level--)
        {
            // Make sure the level has been processed
            {
                std::unique_lock<std::mutex> lock(upsweep_mutex);
                upsweep_cv.wait(lock, [&level_processed, &level] { return level_processed[level] == 1; });
            }

            // Prepare the local offdiagonal for communication
            events.streamWaitEvent<hw>(H2OpusUpsweepEvent, stream, level);
            TDistributedSendRecvBuffer<hw> *comm_buffer = low_rank_exchange_helpers[level].comm_buffer;
            distributed_horthog_prepare_offDiag_template<hw>(basis_branch, v_basis_tree, comm_buffer, dist_workspace,
                                                             level, stream);
            events.recordEvent<hw>(H2OpusBufferDownEvent, level, stream);
            events.synchEvent<hw>(H2OpusBufferDownEvent, level);

            // Send and receive all data
            horthogExecuteLowRankExchange(&low_rank_exchange_helpers[level]);
        }
    }

    if (phase != 0)
    {
        for (int level = num_levels - 1; level >= 1; level--)
        {
            TDistributedSendRecvBuffer<hw> *comm_buffer = low_rank_exchange_helpers[level].comm_buffer;
            horthogExecuteLowRankWait(&low_rank_exchange_helpers[level]);

            // Send the received offdiagonal data to the device
            comm_buffer->prepareRecvBuffer(stream);
            events.recordEvent<hw>(H2OpusBufferUpEvent, level, stream);
        }
    }
}

template <int hw>
void distributed_horthog_upsweep_template(TBasisTree<hw> &basis_branch, HorthogWorkspace &branch_workspace,
                                          H2OpusEvents &events, std::mutex &upsweep_mutex,
                                          std::condition_variable &upsweep_cv, std::vector<int> &level_processed,
                                          MPI_Comm comm, h2opusComputeStream_t stream)
{
    //////////////////////////////////////////////////////////////////////////////////////////
    // Do the upsweep on each branch
    //////////////////////////////////////////////////////////////////////////////////////////
    int num_levels = basis_branch.depth;

#ifdef H2OPUS_PROFILING_ENABLED
    Timer<hw> timer;
    timer.init();
    PerformanceCounter::clearCounters();

    timer.start();
#endif

    horthog_upsweep_leaves(basis_branch, branch_workspace, branch_workspace.Tu_hat, stream);

#ifdef H2OPUS_PROFILING_ENABLED
    HLibProfile::cumRun(HLibProfile::HORTHOG_BASIS_LEAVES, PerformanceCounter::getOpCount(), timer.stop());
    PerformanceCounter::clearCounters();
#endif

    events.recordEvent<hw>(H2OpusUpsweepEvent, num_levels - 1, stream);

    // Notify the low rank exchange thread that the level has been processed
    {
        std::lock_guard<std::mutex> lock(upsweep_mutex);
        level_processed[num_levels - 1] = 1;
    }
    upsweep_cv.notify_one();

#ifdef H2OPUS_PROFILING_ENABLED
    timer.start();
#endif

    for (int level = num_levels - 2; level >= 0; level--)
    {
        // Upsweep for this level
        // Executed on the main stream
        horthog_upsweep_level(basis_branch, branch_workspace, branch_workspace.Tu_hat, branch_workspace.u_new_ranks,
                              level, stream);
        events.recordEvent<hw>(H2OpusUpsweepEvent, level, stream);

        // Notify the low rank exchange thread that the level has been processed
        {
            std::lock_guard<std::mutex> lock(upsweep_mutex);
            level_processed[level] = 1;
        }
        upsweep_cv.notify_one();
    }

#ifdef H2OPUS_PROFILING_ENABLED
    HLibProfile::cumRun(HLibProfile::HORTHOG_UPSWEEP, PerformanceCounter::getOpCount(), timer.stop());
    PerformanceCounter::clearCounters();
#endif
}

template <int hw>
void distributed_horthog_prepare_comm_buffers_template(TDistributedHMatrix<hw> &dist_hmatrix,
                                                       DistributedHorthogWorkspace &dist_workspace,
                                                       distributedH2OpusHandle_t dist_h2opus_handle)
{
    TDistributedCompresseBasisTree<hw> &compressed_tree = dist_hmatrix.compressed_basis_tree_data;
    TBasisTree<hw> &u_branch = dist_hmatrix.basis_tree.basis_branch;
    HorthogWorkspace &branch_workspace = dist_workspace.branch_workspace;

    // Make sure we have enough buffers to accommodate the depth of this hierarchical matrix
    dist_h2opus_handle->allocateBuffers<hw>(u_branch.depth + 1);

    TDistributedTransferBuffer<hw> &gather_buffer = dist_h2opus_handle->getGatherBuffer<hw>();
    TDistributedTransferBuffer<hw> &scatter_buffer = dist_h2opus_handle->getScatterBuffer<hw>();

    // Upsweep gather buffer
    int top_te_rows, top_te_cols;
    distributed_horthog_get_level_projection_dims(u_branch.level_data, branch_workspace, 0, top_te_rows, top_te_cols);

    size_t top_elements = top_te_rows * top_te_cols;
    size_t branch_elements = 0;
    if (dist_h2opus_handle->rank == 0)
        branch_elements = top_elements * dist_h2opus_handle->num_ranks;

    gather_buffer.setBufferSizes(top_elements * sizeof(H2Opus_Real), branch_elements * sizeof(H2Opus_Real));
    gather_buffer.setComm(dist_h2opus_handle->commgather);

    // Downsweep scatter buffer
    branch_elements = top_elements;
    top_elements = 0;
    if (dist_h2opus_handle->rank == 0)
        top_elements = branch_elements * dist_h2opus_handle->num_ranks;

    scatter_buffer.setBufferSizes(top_elements * sizeof(H2Opus_Real), branch_elements * sizeof(H2Opus_Real));
    scatter_buffer.setComm(dist_h2opus_handle->commscatter);

    // Low rank buffers
    int depth = u_branch.depth;
    for (int level = 1; level < depth; level++)
    {
        TDistributedSendRecvBuffer<hw> &comm_buffer = dist_h2opus_handle->getSendRecvBuffer<hw>(level + 1);
        TDistributedCompressedBSNData<hw> &bsn_data = compressed_tree.coupling_compressed_bsn_data[level];

        int level_te_rows, level_te_cols;
        distributed_horthog_get_level_projection_dims(u_branch.level_data, branch_workspace, level, level_te_rows,
                                                      level_te_cols);

        size_t level_elements = level_te_rows * level_te_cols;
        size_t nodes_to_send = bsn_data.send_process_nodes.size();
        size_t node_to_receive = bsn_data.receive_process_nodes.size();

        comm_buffer.allocateBuffers(nodes_to_send * level_elements * sizeof(H2Opus_Real),
                                    node_to_receive * level_elements * sizeof(H2Opus_Real));

        comm_buffer.setRequests(bsn_data.send_process_ids.size(), bsn_data.receive_process_ids.size());
        comm_buffer.setComm(dist_h2opus_handle->comm);
        comm_buffer.setTag(dist_h2opus_handle->getNewTag());
    }
}

template <int hw>
void distributed_horthog_template(TDistributedHMatrix<hw> &dist_hmatrix, distributedH2OpusHandle_t dist_h2opus_handle)
{
    // If we are profiling, initialize run here and partially sum performances
#ifdef H2OPUS_PROFILING_ENABLED
    HLibProfile::addRun(HLibProfile::HORTHOG_STITCH, 0, 0);
    HLibProfile::addRun(HLibProfile::HORTHOG_PROJECTION, 0, 0);
    HLibProfile::addRun(HLibProfile::HORTHOG_UPSWEEP, 0, 0);
    HLibProfile::addRun(HLibProfile::HORTHOG_BASIS_LEAVES, 0, 0);
#endif

    if (!dist_h2opus_handle->active)
        return;

    h2opusHandle_t h2opus_handle = dist_h2opus_handle->handle;
    h2opusHandle_t top_level_handle = dist_h2opus_handle->top_level_handle;

    H2OpusEvents &events = h2opus_handle->getEvents();
    h2opusComputeStream_t main_stream = h2opus_handle->getMainStream();
    h2opusComputeStream_t secondary_stream = h2opus_handle->getSecondaryStream();
    // h2opusComputeStream_t low_priority_stream = h2opus_handle->getLowPriorityStream();

    //////////////////////////////////////////////////////////////////////////////////////////
    // Workspace allocation
    //////////////////////////////////////////////////////////////////////////////////////////
    H2OpusWorkspaceState ws_needed, top_level_ws_needed;
    distributed_horthog_workspace(dist_hmatrix, ws_needed, top_level_ws_needed, dist_h2opus_handle);

    H2OpusWorkspaceState ws_allocated = h2opus_handle->getWorkspaceState();
    H2OpusWorkspaceState top_level_ws_allocated = top_level_handle->getWorkspaceState();

    if (ws_allocated < ws_needed)
        h2opus_handle->setWorkspaceState(ws_needed);

    if (top_level_ws_allocated < top_level_ws_needed)
        top_level_handle->setWorkspaceState(top_level_ws_needed);

    DistributedHorthogWorkspace dist_workspace;
    distributed_horthog_get_workspace(dist_hmatrix, dist_workspace, dist_h2opus_handle);

    // Prepare all communication buffers
    distributed_horthog_prepare_comm_buffers_template<hw>(dist_hmatrix, dist_workspace, dist_h2opus_handle);

    TDistributedTransferBuffer<hw> &gather_buffer = dist_h2opus_handle->getGatherBuffer<hw>();

    // Allocate events
    TBasisTree<hw> &basis_branch = dist_hmatrix.basis_tree.basis_branch;
    int branch_depth = basis_branch.depth;
    events.allocateEvents<hw>(H2OpusUpsweepEvent, branch_depth);
    events.allocateEvents<hw>(H2OpusBufferUpEvent, branch_depth);
    events.allocateEvents<hw>(H2OpusBufferDownEvent, branch_depth);
    events.allocateEvents<hw>(H2OpusCommunicationEvent, H2OPUS_DIST_HORTHOG_TOTAL_EVENTS);

    //////////////////////////////////////////////////////////////////////////////////////////
    // Helper structs for the streaming callbacks
    //////////////////////////////////////////////////////////////////////////////////////////
    std::vector<HorthogLowRankExchangeHelper<hw>> low_rank_exchange_helpers(branch_depth);
    TDistributedCompresseBasisTree<hw> &compressed_tree = dist_hmatrix.compressed_basis_tree_data;

    for (int level = 1; level < branch_depth; level++)
    {
        TDistributedCompressedBSNData<hw> &bsn_data = compressed_tree.coupling_compressed_bsn_data[level];
        TDistributedSendRecvBuffer<hw> &comm_buffer = dist_h2opus_handle->getSendRecvBuffer<hw>(level + 1);

        int proj_node_rows, proj_node_cols;

        distributed_horthog_get_level_projection_dims(basis_branch.level_data, dist_workspace.branch_workspace, level,
                                                      proj_node_rows, proj_node_cols);

        low_rank_exchange_helpers[level].init(&bsn_data, &comm_buffer, proj_node_rows, proj_node_cols);
    }

    // See if we can use customized threaded communications
    bool usethreads = dist_h2opus_handle->getUseThreads<hw>();

    //////////////////////////////////////////////////////////////////////////////////////////
    // Branch upsweep
    // Stream: Main
    // Stream: Secondary for transfers
    //////////////////////////////////////////////////////////////////////////////////////////
    // Mutex and condition variable for the levels of the upsweep
    std::mutex upsweep_mutex;
    std::condition_variable upsweep_cv;
    std::vector<int> level_processed(branch_depth, 0);
    std::thread *low_rank_exchange_thread = nullptr, *root_branch_thread = nullptr;

    // Initiate the transfers using threads
    if (usethreads)
        low_rank_exchange_thread = new std::thread(
            distributed_horthog_low_rank_exchange<hw>, std::ref(basis_branch), std::ref(compressed_tree),
            std::ref(dist_workspace), std::ref(events), std::ref(low_rank_exchange_helpers), std::ref(upsweep_mutex),
            std::ref(upsweep_cv), std::ref(level_processed), dist_h2opus_handle->local_rank, secondary_stream, -1);

    distributed_horthog_upsweep_template<hw>(basis_branch, dist_workspace.branch_workspace, events, upsweep_mutex,
                                             upsweep_cv, level_processed, dist_h2opus_handle->comm, main_stream);

    // Initiate the transfers when not using threads
    if (!usethreads)
    {
        distributed_horthog_low_rank_exchange(basis_branch, compressed_tree, dist_workspace, events,
                                              low_rank_exchange_helpers, upsweep_mutex, upsweep_cv, level_processed,
                                              dist_h2opus_handle->local_rank, secondary_stream, 0);
    }

    if (usethreads)
        root_branch_thread =
            new std::thread(distributed_horthog_process_toplevel_template<hw>, std::ref(dist_hmatrix),
                            std::ref(dist_workspace), std::ref(events), std::ref(gather_buffer), secondary_stream,
                            dist_h2opus_handle->rank, dist_h2opus_handle->local_rank);

    //////////////////////////////////////////////////////////////////////////////////////////
    // Project the diagonal coupling matrices
    // Stream: Main
    //////////////////////////////////////////////////////////////////////////////////////////
    distributed_horthog_project_diagonal_template<hw>(dist_hmatrix.hnodes, dist_hmatrix.basis_tree, dist_workspace,
                                                      main_stream);

    //////////////////////////////////////////////////////////////////////////////////////////
    // Project the offdiagonal coupling matrices
    // Stream: Main
    //////////////////////////////////////////////////////////////////////////////////////////
    if (usethreads)
        low_rank_exchange_thread->join();
    else
    {
        distributed_horthog_low_rank_exchange(basis_branch, compressed_tree, dist_workspace, events,
                                              low_rank_exchange_helpers, upsweep_mutex, upsweep_cv, level_processed,
                                              dist_h2opus_handle->local_rank, secondary_stream, 1);
    }
    distributed_horthog_project_offdiagonal_template<hw>(
        dist_hmatrix.hnodes, dist_hmatrix.basis_tree, low_rank_exchange_helpers, dist_workspace, events, main_stream);

    // Make sure the top level has been processed before returning
    if (usethreads)
        root_branch_thread->join();
    else
    {
        // We do a blocking gather here
        distributed_horthog_process_toplevel_template(dist_hmatrix, dist_workspace, events, gather_buffer,
                                                      secondary_stream, dist_h2opus_handle->rank,
                                                      dist_h2opus_handle->local_rank);
    }

    BasisTreeLevelData &branch_level_data = dist_hmatrix.basis_tree.basis_branch.level_data;

    branch_level_data.setLevelRanks(vec_ptr(dist_workspace.branch_workspace.u_new_ranks));
    dist_hmatrix.hnodes.diagonal_block.level_data.setRankFromBasis(branch_level_data, 0);
    dist_hmatrix.hnodes.off_diagonal_blocks.level_data.setRankFromBasis(branch_level_data, 0);

    if (dist_h2opus_handle->rank == 0)
    {
        dist_hmatrix.basis_tree.top_level.level_data.setLevelRanks(
            vec_ptr(dist_workspace.top_level_workspace.u_new_ranks));
        dist_hmatrix.hnodes.top_level.level_data.setRankFromBasis(dist_hmatrix.basis_tree.top_level.level_data, 0);
    }
    delete low_rank_exchange_thread;
    delete root_branch_thread;
}

/////////////////////////////////////////////////////////////////////////////////////////////////////////
// Interface routines
/////////////////////////////////////////////////////////////////////////////////////////////////////////
// GPU
/////////////////////////////////////////////////////////////////////////////////////////////////////////
#ifdef H2OPUS_USE_GPU
void distributed_horthog(DistributedHMatrix_GPU &dist_hmatrix, distributedH2OpusHandle_t dist_h2opus_handle)
{
    distributed_horthog_template<H2OPUS_HWTYPE_GPU>(dist_hmatrix, dist_h2opus_handle);
}
#endif

/////////////////////////////////////////////////////////////////////////////////////////////////////////
// CPU
/////////////////////////////////////////////////////////////////////////////////////////////////////////
void distributed_horthog(DistributedHMatrix &dist_hmatrix, distributedH2OpusHandle_t dist_h2opus_handle)
{
    distributed_horthog_template<H2OPUS_HWTYPE_CPU>(dist_hmatrix, dist_h2opus_handle);
}
