...
static int alloc_rule_pts(rule *r, unsigned num_regions)
{
    if (num_regions > r->num_regions)
    {
        ...
        r->pts = (double *)malloc(sizeof(double) * (num_regions * r->num_points * (r->dim + r->fdim)));
...
static rule *make_rule(size_t sz, /* >= sizeof(rule) */
                       unsigned dim, unsigned fdim, unsigned num_points, evalError_func evalError, destroy_func destroy)
{
    rule *r;
...
/* Based on rule75genzmalik.cpp in HIntLib-0.0.10: An embedded
   ...
   and A. A. Malik.  See:
         ...
         A. C. Genz and A. A. Malik, "An embedded [sic] family of fully
...
#define VALS(i) v[fdim * (i)]
        for (iR = 0; iR < nR; ++iR)
        {
            ...
            result = R[iR].h.vol *
                     (r->weight1 * val0 + weight2 * sum2 + r->weight3 * sum3 + weight4 * sum4 + r->weight5 * sum5);
...
static rule *make_rule75genzmalik(unsigned dim, unsigned fdim)
{
   ...
   slow and is superseded by other methods (e.g. Monte-Carlo). */
    ...
    r = (rule75genzmalik *)make_rule(sizeof(rule75genzmalik), dim, fdim,
...
static int converged(unsigned fdim, const esterr *ee, double reqAbsError, double reqRelError, error_norm norm)
...
static int rulecubature(rule *r, unsigned fdim, integrand_v f, void *fdata, const hypercube *h, size_t maxEval,
                        double reqAbsError, double reqRelError, error_norm norm, double *val, double *err, int parallel)
{
    size_t numEval = 0;
    heap regions;
    unsigned i, j;
    ...
    while (numEval < maxEval || !maxEval)
    {
        ...
        {                              /* minimize number of function evaluations */
            ...
            if (cut_region(R, R + 1) || eval_regions(2, R, f, fdata, r) || heap_push_many(&regions, 2, R))
                goto bad;
            numEval += r->num_points * 2;
        }
    }
...
static int cubature(unsigned fdim, integrand_v f, void *fdata, unsigned dim, const double *xmin, const double *xmax,
                    size_t maxEval, double reqAbsError, double reqRelError, error_norm norm, double *val, double *err,
                    int parallel)
{
    rule *r;
    hypercube h;
    ...
    status = !h.data ? FAILURE
                     : rulecubature(r, fdim, f, fdata, &h, maxEval, reqAbsError, reqRelError, norm, val, err, parallel);
    destroy_hypercube(&h);
    destroy_rule(r);
    return status;
}

int hcubature_v(unsigned fdim, integrand_v f, void *fdata, unsigned dim, const double *xmin, const double *xmax,
                size_t maxEval, double reqAbsError, double reqRelError, error_norm norm, double *val, double *err)
{
    return cubature(fdim, f, fdata, dim, xmin, xmax, maxEval, reqAbsError, reqRelError, norm, val, err, 1);
}
...
static int fv(unsigned ndim, size_t npt, const double *x, void *d_, unsigned fdim, double *fval)
{
...
int hcubature(unsigned fdim, integrand f, void *fdata, unsigned dim, const double *xmin, const double *xmax,
              size_t maxEval, double reqAbsError, double reqRelError, error_norm norm, double *val, double *err)
{
    int ret;
    fv_data d;
    ...
    d.f = f;
    d.fdata = fdata;
    ret = cubature(fdim, fv, &d, dim, xmin, xmax, maxEval, reqAbsError, reqRelError, norm, val, err, 0);
    return ret;
}
