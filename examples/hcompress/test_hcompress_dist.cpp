...
int main(int argc, char **argv)
{
    ...
    bool check_results =
        arg_parser.flag("c", "check_results", "Check the orthogonalization error and the compression error", false);
    ...
    if (dist_h2opus_handle->active)
    {
        ...
        // Approximate the norm of the distributed hmatrix
        dist_h2opus_handle->setRandSeed(dist_h2opus_handle->rank, H2OPUS_HWTYPE_CPU);
        H2Opus_Real norm_hmatrix =
            distributed_hmatrix_norm<H2Opus_Real, H2OPUS_HWTYPE_CPU>(dist_hmatrix, 10, dist_h2opus_handle);

        // We use absolute thresholds for compression, so scale the relative threshold by the approximate norm
        trunc_eps *= norm_hmatrix;
        if (!proc_rank)
            printf("Approximate distributed hmatrix norm = %e\n", norm_hmatrix);

        // Construct on process 0
        if (!proc_rank && check_results)
        {
            ...
            horthog(full_hmatrix, dist_h2opus_handle->handle);
            hcompress(full_hmatrix, trunc_eps, dist_h2opus_handle->handle);

            H2Opus_Real approx_hmatrix_norm =
                hmatrix_norm<H2Opus_Real, H2OPUS_HWTYPE_CPU>(full_hmatrix, 10, dist_h2opus_handle->handle);
            printf("Approximate sequential hmatrix norm = %e\n", approx_hmatrix_norm);

            // outputEps(full_hmatrix, "structure.eps");
            // printf("New_rank:\n");
            // for(int i = full_hmatrix.u_basis_tree.depth - 1; i >= 0; i--)
            // printf("%d\n", full_hmatrix.u_basis_tree.getLevelRank(i));
            // printf("\n");
        }

        double timer_counter_cpu[2] = {0, 0};
#ifdef H2OPUS_USE_GPU
        double timer_counter_gpu[2] = {0, 0};
#endif

        for (int run = 0; run < nruns; run++)
        {
            // Orthogonalize bases before compression
            ...
            run_time = 0;
            run_time -= MPI_Wtime();
            distributed_hcompress(dist_orthog_hmatrix, trunc_eps, dist_h2opus_handle);
            MPI_Barrier(comm);
            run_time += MPI_Wtime();
            if (!proc_rank)
                printf("Hcompress Run %d: CPU time %fs\n", run, run_time);
            if (run)
                timer_counter_cpu[1] += run_time;

            // if(proc_rank == 0)
            // {
            //     printf("New_rank:\n");
            //     for(int i = dist_orthog_hmatrix.basis_tree.basis_branch.depth - 1; i >= 0; i--)
            //         printf("%d\n", dist_orthog_hmatrix.basis_tree.basis_branch.getLevelRank(i));
            //     printf("\n");
            // }
            ...
            if (check_results)
            {
                ...
                H2Opus_Real compression_diff = dist_vec_diff(vec_ptr(dist_y), vec_ptr(dist_y_orthog), dist_ld, comm);
                if (!proc_rank)
                    printf("  CPU compress error = %e\n", compression_diff);
            }
            ...
            cudaDeviceSynchronize();
            run_time -= MPI_Wtime();
            distributed_hcompress(dist_hmatrix_gpu, trunc_eps, dist_h2opus_handle);
            cudaDeviceSynchronize();
            MPI_Barrier(comm);
            run_time += MPI_Wtime();
            if (!proc_rank)
                printf("Hcompress Run %d: GPU time %fs\n", run, run_time);
            if (run)
                timer_counter_gpu[1] += run_time;
            ...
            if (check_results)
            {
                distributed_hgemv(1, dist_hmatrix_gpu, vec_ptr(dist_x_gpu), dist_ld, 0, vec_ptr(dist_y_gpu), dist_ld, 1,
                                  dist_h2opus_handle);
                thrust::host_vector<H2Opus_Real> gpu_result = dist_y_gpu;
                H2Opus_Real compression_diff = dist_vec_diff(vec_ptr(dist_y), vec_ptr(gpu_result), dist_ld, comm);
                if (proc_rank == 0)
                    printf("  GPU compress error = %e\n", compression_diff);
            }
#endif
        }
        ...
        {
            double avg_time[2];
            avg_time[0] = timer_counter_cpu[0] / (nruns - 1);
            avg_time[1] = timer_counter_cpu[1] / (nruns - 1);
            printf("Average CPU runtimes = horthog %.3f ms, hcompress %.3f ms\n", 1e3 * avg_time[0], 1e3 * avg_time[1]);
        }
        ...
        {
            double avg_time[2];
            avg_time[0] = timer_counter_gpu[0] / (nruns - 1);
            avg_time[1] = timer_counter_gpu[1] / (nruns - 1);
            printf("Average GPU runtimes = horthog %.3f ms, hcompress %.3f ms\n", 1e3 * avg_time[0], 1e3 * avg_time[1]);
        }
#endif
    }
