...
void applyDenseLRU(HMatrix &hmatrix, H2Opus_Real *M, int n, LowRankUpdate &low_rank_update)
{
    ...
    for (int update_id = 0; update_id < num_updates; update_id++)
    {
        ...
        cblas_gemm(CblasNoTrans, CblasTrans, rows, cols, rank, alpha, U, low_rank_update.ldu, V, low_rank_update.ldv,
                   beta, M_block, n);
    }
}
...
int main(int argc, char **argv)
{
    ...
    if (n >= 32768)
        printf("Warning: This example generates the full dense matrix of size %d x %d\n", n, n);
