         ...
         H2Opus_Real &error, h2opusHandle_t h2opus_handle)
{
    ...
    error = rnorm2 / bnorm2;
    it = 0;
    printf("CG iteration %d error %e\n", it, error);

    H2Opus_Real prev_rho;
...
int main(int argc, char **argv)
{
    ...
    H2Opus_Real trunc_eps = arg_parser.option<H2Opus_Real>(
        "te", "trunc_eps", "Relative truncation error threshold for the original hamtrix", 1e-6);
    ...
    // like the hgemv, which will be called a lot during the construction of the preconditioner
    ...
    // Use an adaptive threshold for the constuction of the NS iterates, starting at
