...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
template <class T> struct HcompressUpsweep_Functor
{
    ...
    HcompressUpsweep_Functor(T *T_base, T *E_base, T *TE_base, T **ptr_T, T **ptr_E, T **ptr_TE, int child_new_rank,
    ...
    __host__ __device__ void operator()(const unsigned int &node_index) const
    {
        ...
        for (int child = 0; child < max_children; child++)
        {
            if (child_index != H2OPUS_EMPTY_NODE)
            {
                ...
                ptr_T[child_level_index] = T_base + child_level_index * child_rank * child_rank;
...
inline void hcompress_upsweep_batch_marshal(T *T_base, T *E_base, T *TE_base, T **ptr_T, T **ptr_E, T **ptr_TE,
...
{
    HcompressUpsweep_Functor<T> upsweep_functor(T_base, E_base, TE_base, ptr_T, ptr_E, ptr_TE, child_new_rank,
                                                child_rank, parent_rank, child_start, parent_start, max_children, head,
                                                next);
